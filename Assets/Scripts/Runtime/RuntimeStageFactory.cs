...
{
    /// <summary>
    /// Creates Stage 1 runtime seed data.
    /// </summary>
    public static class RuntimeStageFactory
    {
        ...
        public static StageDefinition CreateStage1()
        {
            ...
            stage.sounds = CreateSounds();
            return stage;
        }

        private static List<ItemDefinition> CreateItems()
        {
            // 아이템 ID는 스프라이트 ID와 퍼즐 required/reward ID에서 함께 사용되므로 문자열을 고정한다.
        ...
        {
            var graph = new MonsterNodeGraph();
        ...
        private static List<SoundEntry> CreateSounds()
        {
            var entries = new List<SoundEntry>();
            // soundId는 InteractableDefinition과 PuzzleDefinition에서 액션으로 참조하는 안정적인 키다.
            entries.Add(new SoundEntry { soundId = "bgm_stage1_ambient", category = SoundCategory.Bgm, loop = true });
            entries.Add(new SoundEntry { soundId = "bgm_final_chase", category = SoundCategory.Bgm, loop = true });
            entries.Add(new SoundEntry { soundId = "ui_click", category = SoundCategory.Ui });
            entries.Add(new SoundEntry { soundId = "sfx_confirm", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_door", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_item_pickup", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_drawer_open", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_drawer_close", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_hide", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_puzzle_success", category = SoundCategory.Sfx });
            entries.Add(new SoundEntry { soundId = "sfx_puzzle_failure", category = SoundCategory.Sfx });
            return entries;
        }
        ...
        private static ItemDefinition Item(string id, string name, ItemType type, string altarSymbol)
        {
            ...
            item.iconResource = "Items/item_" + id;
            return item;
        }
        ...
        private static PuzzleDefinition Puzzle(string id, string name, PuzzleType type, string[] answer, string[] rewards, string flag, string[] requiredItems, float failureDanger = 8f)
        {
            ...
            puzzle.closeUpResource = "Puzzles/" + id;
            return puzzle;
        }
        ...
        {
            var albumClue = Clue("attic_family_album_photo", "Family Album Photo", "CloseUps/attic_family_album_photo", new Rect(0.30f, 0.18f, 0.42f, 0.42f));
            ...
            room.faces = new[]
            {
                ...
                WithBackground(
                    ...
                    new[]
                    {
                        ...
                        {
                            backgroundResource = "Rooms/first_floor_hallway_south_chase",
            ...
            room.faces = new[]
            {
                WithBackground(
                    ...
                    new[]
                    {
                        new ConditionalBackgroundDefinition
                        {
                            backgroundResource = "Rooms/entrance_north_chase",
                            conditions = new ConditionDefinition { requiredFlagIds = new[] { "final_chase_started" } }
                        }
                    }),
        ...
        {
            var seatOrderClue = Clue("dining_seat_order_clue", "Dining Seat Order", "CloseUps/dining_seat_order_clue", new Rect(0.24f, 0.20f, 0.52f, 0.52f));
        ...
        {
            var clockClue = Clue("kitchen_clock_clue", "Kitchen Clock", "CloseUps/kitchen_clock_clue", new Rect(0.38f, 0.16f, 0.24f, 0.30f));
            ...
            hide.hideViewResource = "HideViews/kitchen_sink_hide_view";
        ...
        private static RoomDefinition LaundryRoom()
        {
            ...
            hide.hideViewResource = "HideViews/laundry_machine_hide_view";
            hide.showWorldImage = false;
        ...
        {
            var wallSymbols = Clue("basement_wall_symbols", "Basement Wall Symbols", "CloseUps/basement_wall_symbols", new Rect(0.26f, 0.16f, 0.48f, 0.42f));
            ...
            hide.hideViewResource = "HideViews/basement_main_hide_view";
            hide.showWorldImage = false;
            ...
            room.faces = new[]
            {
                WithBackground(
                    ...
                    new[]
                    {
                        new ConditionalBackgroundDefinition
                        {
                            backgroundResource = "Rooms/altar_room_north_key_taken",
                            conditions = new ConditionDefinition { requiredItemIds = new[] { "front_door_key" } }
                        },
                        new ConditionalBackgroundDefinition
                        {
                            backgroundResource = "Rooms/altar_room_north_key_spawned",
        ...
        private static RoomDefinition ChildRoom()
        {
            ...
            var deskSurface = Clue("child_desk_surface", "Child Desk Surface", "CloseUps/child_desk_surface", new Rect(0.14f, 0.22f, 0.26f, 0.22f));
            var drawingBoard = Clue("child_drawing_board", "Child Drawing Board", "CloseUps/child_drawing_board", new Rect(0.45f, 0.35f, 0.2f, 0.28f));
            var windowView = Clue("child_window_view", "Child Window View", "CloseUps/child_window_view", new Rect(0.08f, 0.2f, 0.19f, 0.62f));
            ...
            room.faces = new[]
            {
                WithBackground(Face("child_room", RoomFaceDirection.North, "Desk Wall", new[] { drawer, deskSurface, drawingBoard }), "Rooms/child_room_north_drawer_empty", new ConditionDefinition { requiredItemIds = new[] { "torn_drawing_fragment" } }),
        ...
        private static RoomDefinition Study()
        {
            ...
            safe.closeUpClosedResource = "CloseUps/study_safe_locked";
            safe.closeUpOpenWithItemResource = "CloseUps/study_safe_open_with_item";
            safe.closeUpOpenEmptyResource = "CloseUps/study_safe_open_empty";
            ...
            clue.clueViewResource = "CloseUps/study_safe_clue_note";
            clue.normalizedHitbox = new Rect(0.33f, 0.45f, 0.36f, 0.34f);
            clue.showWorldImage = false;

            var safeSurrounding = Clue("study_safe_surrounding", "Study Safe Surrounding", "CloseUps/study_safe_surrounding", new Rect(0.43f, 0.16f, 0.36f, 0.68f));
            var deskSurface = Clue("study_desk_surface", "Study Desk Surface", "CloseUps/study_desk_surface", new Rect(0.2f, 0.2f, 0.56f, 0.38f));
            var clueBoard = Clue("study_clue_board", "Study Clue Board", "CloseUps/study_clue_board", new Rect(0.34f, 0.47f, 0.3f, 0.34f));
            var portrait = Clue("study_portrait", "Study Portrait", "CloseUps/study_portrait", new Rect(0.32f, 0.48f, 0.16f, 0.28f));
            var windowView = Clue("study_window_view", "Study Window View", "CloseUps/study_window_view", new Rect(0.03f, 0.21f, 0.22f, 0.58f));
        ...
        {
            var mirrorRuleClue = Clue("bathroom_mirror_rule_clue", "Mirror Reversal Rule", "CloseUps/bathroom_mirror_rule_clue", new Rect(0.30f, 0.24f, 0.40f, 0.48f));
        ...
        {
            var colorSequenceClue = Clue("dressing_color_sequence_clue", "Color Sequence Clue", "CloseUps/dressing_color_sequence_clue", new Rect(0.24f, 0.18f, 0.52f, 0.56f));
        ...
        {
            return "Rooms/" + roomId + "_" + direction.ToString().ToLowerInvariant();
        }
        ...
        private static InteractableDefinition I(string id, string name, InteractableType type, string grantsItemId = "", string targetRoomId = "", string puzzleId = "", string eventId = "")
        {
            var interactable = new InteractableDefinition
            {
                ...
                puzzleId = puzzleId,
                eventId = eventId,
                imageResource = "Objects/" + id,
                normalizedHitbox = HitboxFor(id),
                soundId = SoundFor(type),
                oneShot = type == InteractableType.ItemPickup,
                showWorldImage = false
            };

            if (id == "child_desk_drawer")
            {
                interactable.closeUpClosedResource = "CloseUps/child_desk_drawer_closed";
                interactable.closeUpOpenWithItemResource = "CloseUps/child_desk_drawer_open_with_item";
                interactable.closeUpOpenEmptyResource = "CloseUps/child_desk_drawer_open_empty";
            ...
            {
                interactable.hideViewResource = "HideViews/child_bed_under_view";
            }
