# Step 7: item-broken-hand-mirror-resource

## Game Unit

- unit_type: resource
- unit_id: item_broken_hand_mirror.png
...
- Prior ready-for-review resource unit `item_fuse.png` was approved by the user instruction to proceed according to the Harness.
- Item icon status before this unit is 4 final and 6 placeholder.
- The next item icon resource is `item_broken_hand_mirror.png`.
- Existing target path: `EscapeFromNightmares/Assets/Sprites/Items/item_broken_hand_mirror.png`.
- Current manifest status before implementation: `placeholder`.

## Pre-Implementation Proposal

Create the final inventory icon for `item_broken_hand_mirror.png` as a Unity-importable PNG. This resource represents the broken hand mirror obtained after the mirror puzzle chain and later used as an altar-related item clue.

Resource requirements:

- Output path: `EscapeFromNightmares/Assets/Sprites/Items/item_broken_hand_mirror.png`
- Target display size: square item icon, normalized to `256x256`
- Style: dark hand-drawn 2D, cracked reflective glass, old tarnished handle, thin uneasy lines, muted grayscale with weak red accent only if needed
- Subject: a small broken hand mirror with fractured glass shards still in frame, worn handle, dusty surface, and an unsettling reflection without depicting a character
...
- 2026-05-19: Scoped the step to one resource file only: `item_broken_hand_mirror.png`.
...
- 2026-05-19: Replaced only `item_broken_hand_mirror.png` and preserved the existing Unity `.meta` file.
...
- `item_broken_hand_mirror.png` exists at the Unity target path.
- PNG dimensions are `256x256`.
- File is non-zero byte and loadable as an image.
- `resource_manifest.json` marks only `item_broken_hand_mirror.png` as `final`.
- Existing final item and audio statuses remain unchanged.

## Validation Results

- PASS: `EscapeFromNightmares/Assets/Sprites/Items/item_broken_hand_mirror.png` exists.
- PASS: Image loads via `System.Drawing`.
- PASS: Dimensions are `256x256`.
- PASS: File is non-zero byte: `91497` bytes.
- PASS: `resource_manifest.json` changed `item_broken_hand_mirror.png` from `placeholder` to `final`.
...
- `phases/0-resource-inventory/step7.md`
- `phases/0-resource-inventory/index.json`
- `resource_manifest.json`
- `EscapeFromNightmares/Assets/Sprites/Items/item_broken_hand_mirror.png`
- `EscapeFromNightmares/Assets/Sprites/Items/item_broken_hand_mirror.png.meta`
